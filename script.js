...
const whatsappNumber = '5593984075238';
const whatsappButtons = document.querySelectorAll('.cta-nav');

whatsappButtons.forEach((button) => {
  button.addEventListener('click', () => {
    button.classList.remove('is-clicked');
    requestAnimationFrame(() => button.classList.add('is-clicked'));
    setTimeout(() => button.classList.remove('is-clicked'), 750);
  });
});

menuCards.forEach((card) => {
  const productName = card.querySelector('h3')?.textContent.trim();
  const orderLink = card.querySelector('.menu-meta a');
  if (!productName || !orderLink) return;

  const message = `Ola, gostaria de fazer pedido de um ${productName}.`;
  orderLink.href = `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(message)}`;
...
// Rolagem da fileira de categorias com o mouse (desktop) e roda do mouse.
// No mobile/iOS/Android o toque já rola nativamente pois o CSS usa overflow-x: auto.
const categoryRow = document.querySelector('.category-pill-row');
 
if (categoryRow) {
  // Converte o scroll vertical do mouse (roda) em rolagem horizontal ao passar por cima
  categoryRow.addEventListener('wheel', (event) => {
  ...
  // Arrastar com o mouse para rolar (clicar e arrastar para os lados)
  let isDown = false;
  let startX = 0;
  ...
  categoryRow.addEventListener('mousedown', (event) => {
    ...
    startScrollLeft = categoryRow.scrollLeft;
  });
  ...
  categoryRow.addEventListener('mouseleave', stopDragging);
  ...
  categoryRow.addEventListener('mousemove', (event) => {
    ...
    categoryRow.scrollLeft = startScrollLeft - walk;
  });
 
  // Evita que um arraste dispare o clique/filtro do botão
